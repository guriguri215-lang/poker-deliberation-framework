...
- source SHA-256: `9656a57d64fe074906e5d62cd76590baff187b7d937d95fb8eb3e7c527b19b52`
...
| `P2-010A` | `RM-010` | `completed` | `RM-006`, `RM-007`, `RM-023`, `P2-024A` | Typed pure phase services with serial execution and no persistence integration. | Typed pure phase services are implemented; durable integration remains in P2-010B. |
| `P2-011A` | `RM-011` | `not_started` | `RM-023`, `P2-010A` | Strict budget schema, fake clock, serial accounting, and retry classification. | Not started. |
| `P2-027A` | `RM-027` | `not_started` | `RM-023`, `P2-024A` | Classification, retention, expiry, quarantine, and disposition policy/schema only. | Not started. |
| `P2-012A` | `RM-012` | `not_started` | `RM-023`, `P2-010A`, `P2-011A`, `P2-027A` | Immutable revision storage, manifest, transaction, lock, recovery claim, and revision CAS foundation. | Not started. |
| `P2-010B` | `RM-010` | `not_started` | `P2-012A` | Phase service integration with durable transition ordering and fault injection. | Not started. |
| `P2-011B` | `RM-011` | `not_started` | `P2-012A` | Durable usage/resume, reservations, concurrency, cancellation, and RM-028 interface. | Not started. |
| `P2-012B` | `RM-012` | `not_started` | `P2-010B`, `P2-011B`, `P2-012A` | Completion marker, verified reader/status mapping, migration, and lifecycle hooks. | Not started. |
| `P2-013A` | `RM-013` | `not_started` | `P2-012B` | Approval actor, authority, action digest, request/decision idempotency, and CAS transaction. | Not started. |
| `P2-027B` | `RM-027` | `not_started` | `P2-012B`, `P2-013A` | Authorized cleanup executor, dry-run digest, CAS, receipt, tombstone, and reconciliation. | Not started. |
| `P2-013B` | `RM-013` | `not_started` | `P2-013A`, `P2-027B` | Resume integration, legacy reissue, expiry/revocation, and lifecycle binding. | Not started. |
...
| `RM-010` | Orchestrator phase services | `phase-2` | `P1` | `in_progress` | `RM-006`, `RM-007`, `RM-023`, `RM-024` | `P2-010B` | `required` |
| `RM-011` | Budget, retry, timeout, cancellation, and concurrency semantics | `phase-2` | `P1` | `planned` | `RM-023`, `RM-024` | `P2-011B` | `required` |
| `RM-012` | Versioned run manifest and failure atomicity | `phase-2` | `P1` | `planned` | `RM-023`, `RM-024` | `P2-012B` | `required` |
| `RM-013` | Approval and resume contract hardening | `phase-2` | `P1` | `planned` | `RM-012`, `RM-024` | `P2-013B` | `required` |
...
| `RM-027` | Local data lifecycle | `phase-2` | `P1` | `proposed` | `RM-023`, `RM-024` | `P2-027B` | `required` |
...
- Status: `in_progress`
- Status reason: P2-010A is completed with contract-bound evidence; RM-010 remains in progress because P2-010B is separately unapproved and not started.
...
- Status: `planned`
- Status reason: Current ordinary orchestration does not enforce several declared fields; implementation is not started.
...
- Targets:
  ...
  - provider and tool execution boundaries
- Acceptance criteria:
  - Unknown, zero, or over-cap cost prevents external calls while free local execution remains valid; retries are typed and idempotent; unconfirmed cancellation is not success.
- Tests:
  - fake-clock budget tests
  - retry classifier tests
  - concurrency/cost reservation tests
  - cooperative and uncooperative cancel tests
- Decision gate rationale:
...
- Status: `planned`
- Status reason: The current v1 marker is not validated and whole-run completion is not atomic; implementation is not started.
...
- Tests:
  - write-boundary fault injection
  - corrupt/missing/hash mismatch
  - lock/recovery
  - old/future version migration
- Decision gate rationale:
...
- Status: `planned`
- Status reason: Current local approvals have no actor, revision, CAS, or action digest; implementation is not started.
...
- Tests:
  ...
  - actor/authority/action binding
- Decision gate rationale:
  - actor trust model
...
- Objective: Implement the approved P2-024A attempt-scoped context policy, immutable envelope, lineage, expiry, integrity, and provider allowlist contract without persistence, cleanup, external runtime, or later Phase 2 work.
- Capabilities:
  - none
- Targets:
...
- Status: `proposed`
- Status reason: Approved as a roadmap candidate only; current local artifacts have no lifecycle policy.
- Objective: Define versioned classification, retention, expiry, disposition, recovery, and audit receipts for local artifacts and contexts.
- Capabilities:
  - none
- Targets:
  - runs storage
  ...
  - future cleanup CLI
- Acceptance criteria:
  - RM-027A freezes policy/schema before storage; RM-027B runs only after RM-012/P2-013A and requires actor authority, an approved dry-run action digest, revision CAS, active/pending protection, idempotent receipts/tombstones, and partial-failure reconciliation without claiming secure erase absent platform evidence.
- Tests:
...
- Relations:
  - RM-027A provides policy/schema before RM-012; destructive RM-027B follows RM-012 and P2-013A authorization.
