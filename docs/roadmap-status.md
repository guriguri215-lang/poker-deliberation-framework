...
- source SHA-256: `3ab5dd6c4d498be4501a4e36c195a0ac066a211d8748be06d8bb10e572c17028`
...
| `P2-025B` | `RM-025` | `completed` | `P2-024A`, `P2-025A`, `P3-017A`, `P3-030C` | One terminally verified P3-030C river call-or-fold run projected into five fresh, serial, read-only roles with exact outbound confirmation, mode-bound identity, strict structured results, durable additive replay, no calculator delegation, no provider/model fallback, and no external solver; local_only requires no Codex or API, codex_subscription uses saved ChatGPT/Codex authentication, and the optional openai_api adapter remains default-disabled and live-unqualified. | The three explicit runtime/auth modes, bounded bridge contracts, deterministic transports, additive storage and terminal replay, deterministic evaluation, and sealed live-attestation admission are complete. Candidate-bound historical saved-subscription evidence is preserved but is not current authority. Current-tree qualification is derived only from public preflight over the current strict canonical V2 live manifest and its bound deterministic evaluation: both absent is UNKNOWN, an incomplete or invalid/binding-mismatched pair is FAIL, and only a fully passing pair sets subscription_live_qualified=true. The optional API route remains disabled and live-unqualified. |
...
| `P3-030G` | `RM-030` | `completed` | `P2-025B`, `P3-030D`, `P3-030F` | One deterministic production-workflow qualification harness that uses the production P3-030D prepare, twelve-hash confirm, and terminal run path, then the P3-030F preview, exact 17-field confirmation, and single-role execute wrappers for all five fixed roles through a deterministic no-network transport seam; verifies terminal status, replay, report, lineage, and hashes; and emits self-hashed canonical sanitized evidence containing only safe codes, hashes, counts, fixed metrics, and runtime inventory. No live model or network execution, current model/provider qualification, automatic confirmation, retry, repair, fallback, role skip, bulk or parallel execution, raw source, prompt, outbound bytes, credential values, narrative, model trace, solver, GTO/equilibrium, or strategy-quality claim is included. | The repository-owned deterministic production-workflow qualification harness, fixed cases and metrics, deterministic no-network transport seam, self-hashed canonical sanitized evidence manifest, and declared tests are implemented. This is deterministic repository evidence only, not current live model/provider authority. Current-tree qualification is derived only from public preflight over the current strict canonical V2 live manifest and its bound deterministic evaluation: both absent is UNKNOWN, an incomplete or invalid/binding-mismatched pair is FAIL, and only a fully passing pair sets subscription_live_qualified=true. |
...
- Status: `planned`
- Status reason: P2-025B completed a separately named river-only saved-subscription path and preserves candidate-bound historical sealed evidence, but that evidence is not current authority and the bounded path does not start the broad provider milestone. Current-tree qualification is derived only from public preflight over the current strict canonical V2 live manifest and its bound deterministic evaluation: both absent is UNKNOWN, an incomplete or invalid/binding-mismatched pair is FAIL, and only a fully passing pair sets subscription_live_qualified=true. The optional API route remains disabled, deterministic-only, and live-unqualified; RM-019 dependencies and general provider scope remain planned.
...
- Acceptance criteria:
  ...
  - P3-030G drives the production P3-030D prepare, twelve-hash confirm, and terminal run path, then all five fixed P3-030F roles through preview, exact 17-field confirmation, and one execute call per role using only a deterministic no-network transport seam.
  - P3-030G requires its fixed cases and metrics to pass, verifies terminal status, replay, report, lineage, and hashes, and emits a self-hashed canonical result plus qualification manifest whose sanitized evidence is limited to safe codes, hashes, counts, fixed metrics, and runtime inventory.
  - P3-030G never stores raw source, prompt or outbound bytes, credential values, narrative, model trace, or user_materials, and it does not run or qualify a live model/provider, retry or repair a workflow, invoke a solver, establish GTO/equilibrium, or prove strategy quality.
...
- Tests:
  ...
  - tests/unit/test_bounded_river_review_workflow_qualification.py
...
- Decision gate rationale:
  ...
  - Any promotion of P3-030G deterministic no-network evidence to current live model/provider qualification, strategy quality, solver agreement, GTO/equilibrium, automatic retry or repair, or third-party certification
- Completion-time relations (historical; not current status assertions):
  ...
  - P3-030G depends explicitly on P2-025B, P3-030D, and P3-030F and qualifies only their deterministic production-workflow composition through a no-network seam and canonical sanitized evidence; it does not convert deterministic evidence into live qualification or widen model, provider, solver, retry, role, parser, calculator, range, report, or strategy authority.
